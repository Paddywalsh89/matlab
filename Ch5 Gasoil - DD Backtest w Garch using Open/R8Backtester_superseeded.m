...
function R8Backtester(w,shift,gasoil_data,gaoil_data_close,offset,ma)%gaoil_data_close,excel_row,date,time,indicator)
%w=20;
%shift=0;
...
%%if order_type(event_number)=strcmp(A,order_type(event_number)
    if strcmp(A,B)==1;
        ZC(i)=-1;
    order_price(event_number+1)= -order_price(event_number-1) + signal(m-1);
    else
    ZC(i)=1;
    order_price(event_number+1)= order_price(event_number-1) - signal(m-1);
end
...
plot(x,signal,'b-',x,LI,'r-',x,ZC_plot,'g-',x,v1,'m',x,v2,'m');
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%This section will print to the general results Excel spreadsheet. 
filename='R0_Trading strategy results.xlsx'
%Interval% 
%manual enter
%Moving Window
xlswrite(filename,w,1,sprintf('C%d',offset))
%Shift
xlswrite(filename,shift,1,sprintf('D%d',offset))
%Movav
xlswrite(filename,ma,1,sprintf('E%d',offset))
%Entries
xlswrite(filename,Entries_Accuracy,1,sprintf('F%d',offset))
%Exits
xlswrite(filename,Exits_Accuracy,1,sprintf('G%d',offset))
%Profitability
xlswrite(filename,overall_profit_or_loss,1,sprintf('H%d',offset))
%Buys
xlswrite(filename,number_of_buys,1,sprintf('I%d',offset))
%Sells
 xlswrite(filename,number_of_sells,1,sprintf('J%d',offset))