...
function R14Backtester_Volatility_Econometrics (signalName, sp500Date, sp500Index,spGsciIndex,w,shift,data,gasoil_data_close,date,offset,ma,mmdate,returnsSignal,interestFreeRate,interestFreeRateDaily,excel_row,excelSheet)
% R14 10-Apr-18 Moving back to R1 movav AND 
...
% Also add %% 22 to print to Excel.
% R12 26/03/18 contains 1 change only, reverting to movavR0
% the reason for that is R0 contains no snooping (neither does R1) - but R1
% instroduced an extra delay of w for shift >0. 
...
for j=1:m 
    signal(i)=data(j);      %(j+w) chosen as LI starts after w instances. 
...
%% 22 Print Trading Perf to Excel 
%%This section will print to the general results Excel spreadsheet. 
filename='R0_Trading strategy results_Avg_tester_R3.xlsx'
A=4;
toplefts = [excel_row,1];
%xlswrite(filename,w,1,sprintf('excel_columnc',excel_row))
%range = XLSrange(size(A),[4,excel_columnd]);

 Headings={'Signal','w','shift','ma','Winning (%)','Total PnL ($)','ROI (%)','numberOfTRades','Annualised Sharpe'};
 %tMulitester='tMultitester.mat';
 %save(tMulitester,'signalName','w','shift','ma','winningTrades','overall_profit_or_loss','totalPnLAsPercentage','numberOfTrades','sharpeRatio')
 tMultitester = {signalName,w,shift,ma,winningTrades,overall_profit_or_loss,totalPnLAsPercentage,numberOfTrades,sharpeRatio};
 %load('tMultitester.mat');
 TperfAnalysis=[tMultitester;]; 
 range2 = XLSrange(size(A),[1,1]);
 xlswrite(filename,Headings,excelSheet,range2);
 range = XLSrange(size(A),toplefts);
 xlswrite(filename,TperfAnalysis,excelSheet,range);
...
% xlswrite(filename,winningTrades,1,range)
% %Profitability
...
% xlswrite(filename,totalPnLAsPercentage,1,range)
% %Count of Buys+Sells
% range = XLSrange(size(A),[excel_row,excel_columni]);
% xlswrite(filename,numberOfTrades,1,range)
% catch
% end
